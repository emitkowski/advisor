...
If nothing worth noting, return {\"learnings\": []}";
        ...
        foreach ($result['learnings'] ?? [] as $item) {
            ...
            $existing = Learning::where('user_id', $userId)
                ...
                ->limit(100)
                ->get()
                ->first(function ($l) use ($item) {
                    similar_text($l->content, $item['content'], $percent);
                    return $percent > 70;
                });
...
If no projects mentioned, return {\"projects\": []}";
    ...
    private function inferImplicitRating(
        ...
        // Skip if any signal already exists for this session (explicit or implicit)
        $hasSignal = Signal::where('advisor_session_id', $sessionId)->exists();
...
Only analyze the USER messages, not the assistant messages.";
    ...
    /**
     * Generate a short title for the session from the conversation.
     */
    private function generateTitle(AnthropicService $claude, AdvisorSession $session, string $thread): void
    {
        if ($session->title) {
            return;
        }
        ...
        $result = $claude->completeJson(
            'Generate a short title for this conversation. Return only JSON.',
            [[
                'role'    => 'user',
                'content' => "Give this conversation a short, specific title (4-7 words max) that captures the main topic discussed.\n\nConversation:\n{$thread}\n\nReturn JSON: {\"title\": \"...\"}\n\nDo not use generic titles like 'Advisor Session' or 'Conversation'. Be specific.",
            ]]
        );

        if (!empty($result['title'])) {
            $session->update(['title' => substr($result['title'], 0, 120)]);
        }
    }
    ...
    {
        Log::error('ProcessSessionLearning permanently failed after all retries', [
            'session_id' => $this->sessionId,
            'error'      => $e->getMessage(),
        ]);
    }
